...
\subsection{Add force}\label{sec:add-force}
...
\subsection{Diffuse}\label{sec:diffusion}
...
\subsection{Project}\label{sec:project}
...
\end{equation}
where $c= \nu\frac{\Delta t}{h^2}$ and $u^{(k)}_{(i,j)}$ gives the $k$-th iterate of the velocity vector at grid point $(i,j)$ which corresponds to the entry $i+j \cdot n$ in the column vector representation of $u$.\\
...
\section{Vorticity Confinement}\label{sec:vorticity-confinement}
...
\section{Dependence on the Parameters}\label{sec:parameters}
...
\section{Colors and Import of Images}\label{sec:color-photoimport}
...
%
\section{Features of our Application}
First, we want to mention and describe all the (non-default-JavaView) features in the application, i.e. what all the buttons and sliders are for to give the reader a better understanding or motivation of the following chapters.
\begin{itemize}
\item Sliders:
	\begin{itemize}
	\item BlockSize: Edge length of one ``pixel block'' (see \ref{sec:block-size}) in pixels.
	\item Density radius: Radius of the ``pencil'' drawing density into the display (see \ref{splinesChapter}).
	\item Density constant: Controlling, how much smoke is inserted.
	\item Force radius: Radius of the ``force pencil'', i.e. controlling, how big the region is, which is pulled by inserting force with the mouse.
	\item Force constant: Controlling, how much force is inserted.
	\item Buoyancy force: Controlling, how much buoyancy force is inserted (see \ref{sec:add-force}). Can be set to negative values, which makes hot (i.e. dense) smoke sink instead of rise.
	\item Diffusion: Controlling, how much the fluid diffuses (see \ref{sec:diffusion}).
	\item Viscosity: Controlling viscosity parameter of the fluid (see also \ref{sec:parameters}).
	\item Vorticity: Controlling, how much force is inserted, to preserve details lost by vorticity confinement (see \ref{sec:vorticity-confinement}).
	\item Projection Accuracy: Takes only 0 and 1. If set to 1, the solver produces more accurate results (performing multiple projection steps between all other steps) at the cost of longer computation times (see also \ref{sec:project}).
	\end{itemize}
\item Buttons:
	\begin{itemize}
	\item Reset: Resets application to starting conditions (without affecting the size of the application).
	\item Clear Canvas: Clears the canvas from all smoke and sets the fluid to stand still, but keeps all slider values etc. the same.
	\item Flip Color: If color mode is one, this button selects the next color for drawing density.
	\item Import Image: Imports the image ``image.png'' from the same directory as the executable into the canvas. The pixels of the image are treated as colored smoke (see \ref{sec:color-photoimport}).
	\item Freeze: Pauses the fluid to move. Clicking once more wakes it up again.
	\item Color On/Off: As default, the application starts with greyscale smoke. This button turns on color mode, where you can draw smoke of multiple colors into the display. See also the button ``Flip Color''.
	\end{itemize}
\end{itemize}
%
